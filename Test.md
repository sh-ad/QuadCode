...
3. Чистая архитектура: наука откладывания на завтра от QuadCode
      * Продолжаем идею с практической составляющей книг, когда разработчик переходит от дизайна кода к его архитектуре, когда он уже изучил паттерны, 
        ...
        давайте посмотрим, а в реальной разработке, получается ли так сделать. Тут нужны прямо диаграммы классов, желательно, рассуждения о том, 
...
Почему нашей это будет интересно? - Нам нужны либо люди, которые уже имеют опыт работы с высоконагруженными приложениями, либо хотят его получить. Первые будут читать статью потому что всегда интересно, а не зеленее у соседа трава, а вторые потому что у них другого доступа к реальным примерам то и нет...
...
# Вопросы коллегам по теме для написания статьи
Дословно меня просили 
> вопросы к блокам, что можно спросить у наших ребят из ИТ (например, ты приходишь и берешь у них интервью, это те вопросы, которые помогут раскрыть тему статьи
Конкретно в данном случае, я бы запустил книжный клуб, где бы мы читали по одной главе в неделю, обсуждали, как написанное в книге кладётся на стек компании, а там, где ответов бы не было приглашали архитекторов и спрашивали у них.

Схема бы была, примерно такой:
1. Вторник - открытая встреча для всех желающих, приходит кто хочется, я модерирую диалог, обсуждаем одну главу за раз. Заппись кладётся на внутренние ресурсы компании для истории, я готовклю резюме по главе и вопросы, которые подвисли
2. Четверг - запись подкаста/youtube, обсуждаем уже более структурировано, малым количеством человек и отвечаем на вопросы, которые подвисли. Состав может меняться, может быть, как награда для людей активнее высказываться во вторник
3. Всё это преобразуется в статью, где есть ссылки на подкаст/youtube/чат клуба
4. Чат клуба - сквозной публичный чат для всех подобных активностей

### Мы получаем внутреннюю активность и развитие специалистов, внешнюю активность сразу в нескольких направлениях

Если план бы не взлетел, я бы выбрал несколько опытных разработчиков и обсудил с ними по одной главе, итогом будет "3. Реализация"
- Надёжность или масштабируемость, как у нас баланс построен
- Чем защищаемся от аппаратных сбоев
- Почему именно Cassandra
- Используем ли где-то языки запросов

Потом я бы пошёл к командам и строил бы диалог по схеме историй рыбаков "Я вооот такую рыбу поймал. - А я вооооооот такую", тут наберётся байки для 4. Тестирование
- У кого был самый нелепый аппаратный сбой?
- Кто клал БД запросом, который выкачивал половину БД?
- Знаете случаи, когда брали Монгу, а потом понимали, что нужны транзакции?
- Человеко-читаемые или машино-читаемые форматы данных?
- Сюрпризы от Кафки и других брокеров?
- J-значит Json? Какие сюрпризы мы можем получить?
- Почему не используем библиотеки для ускоренной сериализации Json? Если используем, то какие?

Самое сложное "5. Внедрение" - тут нужны либо сторожилы, либо копать беклог
- Особенности json и неизвестное значение Enum в Go
- context.Context и gRPC
- горутины
- бенчмарки
